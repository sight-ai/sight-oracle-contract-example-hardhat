...
npx hardhat task:launchSightFHEVM --network localSightFHEVM
...
sleep 8"; MNEMONIC=$(grep COMPUTE_PROXY_CHAIN_MNEMONIC= .env | cut -d '"' -f 2) npx hardhat task:deploySightFHEVM --network localSightFHEVM;
